...
def TDNProfilePlus(nop, ele: str, span_label: str, p0: Vec3, p1: Vec3):
    if nop == 42:
        profile = [
            [8, 4, 2, 0],
            [8, 0, 4, 2],
    ...
    for i, y in enumerate([75, 125, 175, 225, 1955]):
        for j in range(4):
            ...
            if num != 0:
                ...
                ret += "      0, YES, Y, 0\n"
                ret += "      Y=%i, %i, NO, 0, 0, NONE, , , , \n" % ((p0 + ux * ddx).x, (p0 + ux * ddx).y)
                ret += "      Y=%i, %i, NO, 0, 0, NONE, , , , \n" % ((p1 - ux * ddx).x, (p1 - ux * ddx).y)
                ret += "      Z=%i, %i, NO, 0, 0, NONE, , , , \n" % ((p0 + ux * ddx).x, (p0 + ux * ddx).z + y)
                ret += "      Z=%i, %i, NO, 0, 0, NONE, , , , \n" % ((p1 - ux * ddx).x, (p1 - ux * ddx).z + y)
    return ret
...
if __name__ == "__main__":
    data = pd.read_excel(".\W14W17\BR1模型参数.xlsx", sheet_name="先张参数")
    locs = pd.read_excel(".\W14W17\BR1模型参数.xlsx", sheet_name="坐标", header=None, index_col=0)
    txt = "*TDN-PROFILE\n"
    ...
    for i, row in data.iterrows():
        ...
        ei = ni
        ej = nj - 1
        p0 = Vec3(locs.at[ni, 1], locs.at[ni, 2], locs.at[ni, 3])
        p1 = Vec3(locs.at[nj, 1], locs.at[nj, 2], locs.at[nj, 3])
        txt += TDNProfilePlus(row['nop'], f"{ei}to{ej}", f"{row['Beam']}PRE", p0, p1)
    with open(".\W14W17\W14W17先张束.mct", 'w+', encoding='gbk') as fid:
        fid.write(txt)
