...
class Superstructure:
    ...
    def check_stress(self, girder_n, location, non_cp_sec, cp_section, pre_stressing, post_str_1, post_str_2, isMiddle):
        """
        混凝土应力核查
        :param girder_n: 梁编号
        :param location: 核查位置
        :param non_cp_sec: 非组合截面
        :param cp_section: 组合截面
        :param pre_stressing: 先张
        :param post_str_1: 第一批后张
        :param post_str_2: 第二批后张
        ...
        fcg0 = self.Transfer(girder=girder_n, loc=location, sec=non_cp_sec, pre_stress=pre_stressing)
        fcg0 += self.Erection(sec=non_cp_sec, pre_stress=pre_stressing, fcg=fcg0)
        fcg0 += self.Deck(girder=girder_n, loc=location, sec=non_cp_sec, pre_stress=pre_stressing, fcg=fcg0)
        dfcg0, fcg_TD1 = self.Post1(sec=post1_sect, pre_stress=pre_stressing, T1_stress=post_str_1, fcg=fcg0, isNoCP=not isMiddle)
        fcg0 += dfcg0
        dfcg0, dfcg_T1 = self.Deck2(girder=girder_n, loc=location, sec=deck_sect, pre_stress=pre_stressing, T1_stress=post_str_1, fcg=fcg0, fcg_T1=fcg_TD1, isNoCP=not isMiddle)
        fcg0 += dfcg0
        fcg_TD1 += dfcg_T1
        dfcg0, dfcg_T1, fcg_TD2 = self.Post2(sec=cp_section, pre_stress=pre_stressing, T1_stress=post_str_1, T2_stress=post_str_2, fcg=fcg0, fcg_T1=fcg_TD1)
        fcg0 += dfcg0
        fcg_TD1 += dfcg_T1
        dfcg0, dfcg_T1, dfcg_T2 = self.DW(girder=girder_n, loc=location, sec=cp_section,
                                          pre_stress=pre_stressing, T1_stress=post_str_1, T2_stress=post_str_2,
