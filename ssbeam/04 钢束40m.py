...
if __name__ == '__main__':
    T1z = interp1d([-20, -2, 0, 2, 20], [1.75, 0.315, 0.315, 0.315, 1.75], 'quadratic')
    T2z = interp1d([-20, -2, 0, 2, 20], [1.35, 0.125, 0.125, 0.125, 1.35], 'quadratic')
    T3z = interp1d([-20, -2, 0, 2, 20], [0.95, 0.125, 0.125, 0.125, 0.95], 'quadratic')
    T4z = interp1d([-20, -2, 0, 2, 20], [0.55, 0.125, 0.125, 0.125, 0.55], 'quadratic')

    T3y = interp1d([-20, -10, -4, 0, 4, 10, 20], [0, 0, +0.15, +0.15, +0.15, 0, 0], 'linear')
    T4y = interp1d([-20, -10, -4, 0, 4, 10, 20], [0, 0, -0.15, -0.15, -0.15, 0, 0], 'linear')
...
#    txt = "*TDN-PROFILE\n"
#    all_ele = "2000to2043"  #
#    txt += post_tendon(all_ele, xx, yy1A, zz1, "T1A")
#    txt += post_tendon(all_ele, xx, yy1A, zz2, "T2A")
#    txt += post_tendon(all_ele, xx, yy3A, zz3, "T3A")
#    txt += post_tendon(all_ele, xx, yy4A, zz4, "T4A")
#    # all_ele = "1000to1043"
...
#    with open("TDN.mct", 'w+', encoding='GBK') as fid:
#        fid.write(txt)
#  print(txt)
