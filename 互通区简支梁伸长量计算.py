...
def vetical_33():
    ...
    return [xx+16.5, zz1, zz2, zz3, zz4]
...
def vetical_40():
    ...
    zz3 = [T3z(x) for x in xx]
    zz4 = [T4z(x) for x in xx]
    return [xx+20.0, zz1, zz2, zz3, zz4]


if __name__ == "__main__":
    xx, zz1, zz2, zz3, zz4 = vetical_40()
    Tendons = [zip(xx, zz1), zip(xx, zz2), zip(xx, zz3), zip(xx, zz4), ]
    for Td in Tendons:
        tds = [list(np.array(a) ) for a in Td]  # f = 1
        NoS = 15
        TheL, DuctL, WorkL = get_allLength(tds, NoS, False)
        ...
        elong = sum(d_list[0:int(len(d_list) / 2)]) * 2
        print("%4s: %8.3f , %8.3f , %8.3f" % ("40m", WorkL, DuctL, elong))
