...
def REQPY_single(s, fs, dso, To, T1=0, T2=0, zi=0.05, nit=20, NS=100, baseline=1, plots=1):
    '''
    REQPY_single - CWT based modification of a single component from
    ...
    if plots:
        ...
        plt.figure(figsize=(6.5, 6.5))
        plt.subplot(311);
        plt.plot(t, sosc, 'c', t, ccs, 'b');
        plt.ylabel('acc. [g]')
        plt.subplot(312);
        plt.plot(t, velsc, 'c', t, cvel, 'b');
        plt.ylabel('vel./g')
        plt.subplot(313);
        plt.plot(t, desplsc, 'c', t, cdespl, 'b');
        plt.ylabel('displ./g')
        plt.xlabel('time [s]')
        plt.legend((': scaled', ': matched'), frameon=False, loc='upper right')
        plt.tight_layout()

        limy = 1.06 * np.max(PSAccs)
        auxx = [T1, T1, T2, T2, T1]
        auxy = [0, limy, limy, 0, 0]
